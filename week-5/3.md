...
- 그 외에 잡다한 기능 (Log, Exec) 들도 구현
  (kubectl logs 명령어를 수행하면 `사용자` > `kube-apiserver` > `kubelet` > `CRI` > `containerd` > `Linux /var/log`)
- 의외로 메트릭을 볼 수 있는 방법이 없음 [참고](https://github.com/kubernetes/enhancements/issues/2371) [현재 alpha](https://kubernetes.io/docs/reference/instrumentation/cri-pod-container-metrics/)
...
0. containerd 는 Network Namespace 만들고 ([/var/run/netns](https://github.com/containerd/containerd/blob/v1.7.13/pkg/cri/server/sandbox_run.go#L281-L285))
1. `runc` 로 Pause 컨테이너를 만들고 => PodSandbox
2. [PodSandbox](https://github.com/containerd/containerd/blob/v1.7.13/pkg/cri/server/sandbox_run.go#L328-L338) 에 대하여 [CNI 를 호출해서](https://github.com/containerd/containerd/blob/v1.7.13/pkg/cri/server/sandbox_run.go#L553)
3. CNI 가 주어진 Network Namespace 에 대하여 네트워크를 구성
4. 주어진 Network Namespace 는 이제 네트워크를 사용할 수 있음
...
- Archived 된 프로젝트
- Service Mesh 에 대한 표준 CRD 정의하려고 했지만
- Service Mesh 의 인기가 떨어지, [Gateway API](https://kubernetes.io/docs/concepts/services-networking/gateway/) 개념이 대체
- 비록 사라졌지만 어떤식으로 인터페이스가 정의되고 사용되는지에 대한 예시
